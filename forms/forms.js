...
'use strict';
  ...
  let _go2epa;
	...
	function FormsSewernet(options){
		...
		_formsContents = new FormsContentsSewernet({project_type:_project_type,expected_api_version:_expected_api_version});
		setTimeout(function(){
  ...
  function setToken(token){
    _token = token;
		...
		_eventsModule = new EventsSewernet({project_type:_project_type,token:_token,baseHref:_baseHref,device:_device,expected_api_version:_expected_api_version});
		_eventsModule.on('log',function(mod,data,level,extraData){
			_self.emit("log",mod,data,level,extraData);
		});
		_features = new FeaturesSewernet({project_type:_project_type,token:_token,baseHref:_baseHref,device:_device,expected_api_version:_expected_api_version});
		_features.on('log',function(mod,data,level,extraData){
			_self.emit("log",mod,data,level,extraData);
		});
		_search = new Search({project_type:_project_type,token:_token,baseHref:_baseHref,device:_device,expected_api_version:_expected_api_version});
		...
		_print = new Print({project_type:_project_type,token:_token,baseHref:_baseHref,device:_device,expected_api_version:_expected_api_version});
		...
		_mincut = new Mincut({project_type:_project_type,token:_token,baseHref:_baseHref,device:_device,expected_api_version:_expected_api_version});
		...
		_datesSelector = new DatesSelector({project_type:_project_type,token:_token,baseHref:_baseHref,device:_device,expected_api_version:_expected_api_version,use_tiled_background:_options.use_tiled_background});
    ...
    _go2epa = new Go2Epa({'env':_options.env, 'baseHref':_baseHref, 'device':_device, token:_token});
					...
					if(err && err!=404){
						...
						if(err===404){
							...
							axios.post(_baseHref+'/ajax.sewernet.php', dataToSend).then(function (response) {
								...
								if(response.data.status==="Accepted"){
									cb(null,{table:response.data.message.table,node1:response.data.message.node1[0],node2:response.data.message.node2[0]});
								}else{
  ...
  function setFeatureFile(pol_id,idName,info_type,tableName,photos,deviceTrace){
    _self.emit("log","form.js","setFeatureFile()","info",{'pol_id': pol_id,'idName': idName,'info_type':info_type,'tableName': tableName,'photos': photos,'deviceTrace':deviceTrace});
    return new Promise((resolve, reject) => {
      let dataToSend = {};
      dataToSend.device = _device;
  ...
  function deleteFeatureFile(pol_id,file_id,info_type){
    _self.emit("log","form.js","deleteFeatureFile()","info",{'pol_id': pol_id,'file_id': file_id,'info_type':info_type});
    return new Promise((resolve, reject) => {
      let dataToSend = {};
      dataToSend.device = _device;
      dataToSend.id = pol_id;
  ...
  function getInfoFromPolygon(data){
	...
	***/
	...
	function initFilters(use_tiled_background){
		_filters = new Filters({project_type:_project_type,token:_token,baseHref:_baseHref,device:_device,use_tiled_background:use_tiled_background,expected_api_version:_expected_api_version,strings:_options.strings});
  ...
  FormsSewernet.prototype.clearOfflineVisits            = clearOfflineVisits;
	...
	FormsSewernet.prototype.setgo2epa = ()=>{
  ...
  FormsSewernet.prototype.getInfoFile = (hash)=>{
		...
		var noCache = Math.floor((Math.random() * 100) + 1);
		return axios.get(`${_baseHref}external.getinfofile.php?img=${hash}&noCache=${noCache}`).then(function(result) {
			return result.data;
		})
	};
