...
'use strict';
	...
	let _activeTab          = null;
	let _tabs               = null;
	let _availableTabs      = Array('network','search','hydro','workcat','psector','address','visit');
								...
								for(let f=0;f<retorno[i].fields.length;f++){
									if(retorno[i].fields[f]){
										if(retorno[i].fields[f].type==="combo"){
											...
											let actions = _getTabActions(retorno[i].tabName.toLowerCase(),retorno[i].fields[f].name);
