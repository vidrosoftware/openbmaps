...
export default class Notifications extends EventEmitter{
  ...
  getRoles(){
    return new Promise((resolve, reject) => {
      ...
      axios.post(`${_options.baseHref}/ajax.notifications.php`, data2send).then((response) => {
        _logger.success(_self._fileName, 'getRoles', response.data);
        if(response.data.status==='Accepted'){
          _roles = response.data.message;
          _roles.unshift({'id':-1,'name':_options.strings.SELECT});
          let rolesCombo = _self.createCombo(_roles,{
            'label': _options.strings.ROLES,
            'disabled': false,
            'name': 'roles',
            'widgetAction': 'notificationSelectedRole'
          });
          let usersCombo = _self.createCombo({},{
            'label': _options.strings.USERS,
            'disabled': true,
            'name': 'user',
            'widgetAction': 'notificationSelectedUser'
          });
          let form = _self.buildForm([rolesCombo,usersCombo]);
          resolve(form);
        }else{
      ...
      if(role_id!=-1){
        ...
        axios.post(`${_options.baseHref}/ajax.notifications.php`, data2send).then((response) => {
          ...
          if(response.data.status==='Accepted'){
            ...
            _users.unshift({'id':-1,'name':_options.strings.SELECT});
            let usersCombo = _self.createCombo(_users,{
              'label': _options.strings.USERS,
              'disabled': false,
              'name': 'user',
              'widgetAction': 'notificationSelectedUser',
  ...
  setUser(id){
    _logger.info(_self._fileName, 'setUser',{'id':id});
    _selectedUser = id;
  }
  ...
  buildNotification(subject,data,coordinates,epsg,table,pol_id,id_name){
    return new Promise((resolve, reject) => {
      _logger.info(_self._fileName, 'buildNotification',{'subject':subject,'data':data,'coordinates':coordinates,'epsg':epsg,'table':table,'id_name':id_name,'pol_id':pol_id});
          ...
          for(let i=0;i<data.length;i++){
            if(data[i].label!='the_geom' && data[i].label!='geom' && data[i].type!="button"){
              if(data[i].type==="combo"){
                ...
                if(typeof data[i].value!='undefined') body+= `${data[i].label}: ${data[i].value}\r\n`;
              }
  ...
  _generateLinkToGoogleMaps(coordinates,epsg){
    _logger.info(_self._fileName, '_generateLinkToGoogleMaps',{'coordinates':coordinates,'epsg':epsg});
    proj4.defs("EPSG:25831","+proj=utm +zone=31 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs");
    register(proj4);
    let pt = new Point(coordinates);
    pt.transform(epsg,'EPSG:4326');
    let longLat = toLonLat([pt.getCoordinates()[1],pt.getCoordinates()[0]],'EPSG:4326');
    return `https://maps.google.com/?q=${longLat[0]},${longLat[1]}`;
  }

  _generateLinkToBmaps(table,pol_id,id_name){
    _logger.info(_self._fileName, '_generateLinkToBmaps',{'table':table,'pol_id':pol_id,'id_name':id_name});
