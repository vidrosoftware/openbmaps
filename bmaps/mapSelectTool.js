...
//function mapService($http,$rootScope){
	...
	function init(_map,_epsg,_viewProjection,_viewResolution,_vectorSource,_token,_app_name,_urlWMS,_max_features,_geom_colors,_touchDevice,_mapToc,_mapFactory){
		map								= _map;
		epsg							= _epsg;
	...
	function initPolygonSelect(mode){
		log("initPolygonSelect("+mode+") - isMeasuring: "+isPolygonSelecting);
	...
	function selectPoint(coordinates,mapData,_viewResolution,layerList,feature_id){
		log("selectPoint() multipleSelection: "+multipleSelection+" resolution: "+_viewResolution+" feature_id: "+feature_id,"info",coordinates);
		...
		if(mapData.layersVars.length>0){
			if(typeof mapData.layersVars[mapData.activeLayer]!="undefined"){
				...
				var url = selectableLayer.getSource().getGetFeatureInfoUrl(
												coordinates,  Math.min(Math.max(viewResolution*sensibilityFactor,1),1), viewProjection,
												infoOptionsObj
				);
			}else{
				var url = false;
			...
			infoOptionsObj['QUERY_LAYERS'] = mapToc.getAvailableLayers().toString();
			//add one layer for do getGetFeatureInfoUrl request
			mapFactory.addLayer(mapToc.getRawLayers()[0].Name,0);
			var url = mapFactory.getOLLayersDisplayed()[0].getSource().getGetFeatureInfoUrl(
																coordinates, viewResolution*5, viewProjection,
																infoOptionsObj
								);
			///remove layer
					...
					try{
						if(typeof json.GetFeatureInfoResponse!== 'undefined' || json.GetFeatureInfoResponse!=""){
							if(typeof json.GetFeatureInfoResponse.Layer.length==="undefined"){
								...
								log("GetFeatureInfoResponse from doInfoFromCoordinates - feature_id: "+feature_id,'info',json.GetFeatureInfoResponse);
										...
										if(typeof json.GetFeatureInfoResponse.Layer[i].Feature.Attribute != 'undefined'){
											...
											if(typeof feature_id != 'undefined'){
												if(feauture.Attribute[0].value==feature_id){
													if(mapFactory.getLayersDisplayed().indexOf(json.GetFeatureInfoResponse.Layer[i].name)===-1){
														mapFactory.addLayer(json.GetFeatureInfoResponse.Layer[i].name,i);
														mapFactory.setActiveLayer(json.GetFeatureInfoResponse.Layer[i].name);
													}
	...
	function addresschosen(coordinates){
		...
		addressFeature.setStyle(new ol.style.Style({
															image: new ol.style.Circle({
															radius: 6,
															fill: new ol.style.Fill({
																color: '#72cc33'
	...
	function addInteraction(type) {
		...
		drawEndEvent = draw.on('drawend',function(evt) {
			...
			map.un(pointerMoveListener);
				...
				if(isPolygonSelecting){
					//dispatch event
					$rootScope.$broadcast('polygonSelect',{'polygon':getPolygonSelect()});
				}
		}, this);
	}
	...
	*/
	function highLightGeometry(geometryData,geometryType){
		log("highLightGeometry()","info",{'geometryData':geometryData,'geometryType':geometryType,'highlightedGeoms':highlightedGeoms});
		var format = new ol.format.WKT(),
	...
	function removeGeometryFromMap(feature,index){
		log("removeGeometryFromMap()","info",{'feature':feature,'index':index});
