...
angular.module('app').factory('mapFactory', ['$http','$rootScope','mapMeasureTools','mapSelectTool','mapAddTool','mapOffline','mapAjaxOperations','mapStorage','mapPhotos', function ($http,$rootScope,mapMeasureTools,mapSelectTool,mapAddTool,mapOffline,mapAjaxOperations,mapStorage,mapPhotos) {
	...
	function init(_env,_urlWMS,_urlWMTS,_token,_project,_app_name,_useGeolocation,_max_features,_touchDevice,options){
		env										= _env;
		urlWMS								= _urlWMS;
		urlWMTS								= _urlWMTS;
		token									= _token;
		...
		log("init("+_env+","+_urlWMS+","+_token+","+_project+","+_app_name+","+_useGeolocation+","+_max_features+","+_touchDevice+")","info",options);
		//offline init
		mapOffline.init(urlWMS,touchDevice,localized_strings,project.id);
		if(navigator.onLine){
			log("App online","info");
			appOnline       = true;
		}else{
    ...
    if(navigator.onLine){
  		log("GetCapabilities() online: "+urlWMS+"?SERVICE=WMS&VERSION=1.3.0&REQUEST=GetCapabilities","info");
	...
	function renderMap(){
		...
		var interactions = ol.interaction.defaults({altShiftDragRotate:false, pinchRotate:false, doubleClickZoom: false});
		//sets the map
		map   = new ol.Map({
								target: 			'map',
								layers: 			layers,
		...
		//select
		mapSelectTool.init(map,epsg,viewProjection,viewResolution,vectorSource,token,app_name,urlWMS,max_features,project.geom_colors,touchDevice,mapToc,dataFactory);

		//add
		mapAddTool.init(map,epsg,viewProjection,viewResolution,vectorSource,token,app_name,urlWMS,project.geom_colors,localized_strings);
		...
		map.on('click', function(evt) {
			...
			if(toolSelected==="point"  || toolSelected==="MultiPoint" || toolSelected==="Point"){
				...
				if(!clickDisabled){
					if(appOnline){
						$rootScope.$broadcast('clickedCoordinates',{coordinates:clickedCooordinates});
					}else{
						mapSelectTool.selectPointOffline(evt.pixel,clickedCooordinates,mapToc.getMarkedLayerAsActive());
					}
				}
			}
		});
		...
		map.on('moveend', function (e) {
			...
			$rootScope.$broadcast('mapMoveEnd',{coordinates:clickedCooordinates,'zoom_level':zoom_level,'center':map.getView().getCenter()});
		});

		if(project.use_tiled_background){
			setTiledOverBackground(true,tiled_layer_name,tiled_matrixset);
		};
	...
	function displayLayer(layer_name,reload,item,index){
		...
		if(layers.indexOf(layer_name)>-1){
			...
			if(appOnline){
				...
				_getFeatureTypeFromWFS(layer_name,function(layerAvailableFilters){
					var LayerProperties = mapToc.getLayerProperties(layer_name);
					if(typeof LayerProperties=="undefined"){
						log("displayLayer("+layer_name+") getLayerProperties error","error",LayerProperties);
						return false;
					}
					var source;
					var lay;
					if(LayerProperties.singletile){
						log("displayLayer("+layer_name+") rendered as singleTile","info",LayerProperties);
						//render based on a single image
						source 	= new ol.source.ImageWMS({
									url: 	urlWMS,
									params: {
																	'LAYERS': layer_name,
																	'FILTER': layer_name+':'+getWMSFilters(layer_name,layerAvailableFilters)
																},
						...
						log("displayLayer("+layer_name+") rendered as multiTile","info",LayerProperties);
					...
					mapOffline.readOfflineGeoJSON(project.project_name+"_"+layer_name+".json",function(err,storedGeoJSON){
						...
						if(storedGeoJSON){
							...
							if(features.length>0){
								...
								mapOffline.readOfflineStyle(project.project_name+"_"+layer_name+".js",function(err,storedStyle){
										if(err){
											...
											if(storedStyle){
												...
												log("readOfflineStyle storedStyle","info",storedStyle);
												eval(genericStyles);
												eval(storedStyle);
												layer_name 	= layer_name.split(" ").join("XXX");
												var functionName = "style_"+project.project_name+"_"+layer_name;
												lay      = new ol.layer.Vector({
																		extent: extent,
																		name: layer_name,
																		source: source,
	...
	function createTextStyle(feature, resolution, labelText, labelFont, labelFill, placement) {
	...
	function getLayerAttributes(layer){
		...
		try{
			var url    = urlWMS+"?SERVICE=WFS&VERSION=1.0.0&REQUEST=describeFeatureType&typename="+layer+"&initDate="+initDate+"&endDate="+endDate+"&username="+userName;
			log("url","info",url);
	...
	function doWMSInfo(clickedCooordinates){
		...
		mapSelectTool.selectPoint(clickedCooordinates,getMapData(),map.getView().getResolution());
	}
	...
	function getWMSProjectSettings(setting){
		log("getWMSProjectSettings("+setting+"): "+urlWMS+"?SERVICE=WMS&VERSION=1.3.0&REQUEST=GetProjectSettings","info");
		return $http.get(urlWMS+"?SERVICE=WMS&VERSION=1.3.0&REQUEST=GetProjectSettings", {
			params: {
				json: 1
			}
			...
			log("getWMSProjectSettings("+setting+") - errorCallback","error",response);
			return response;
		});
	}
  ...
  function getBackgroundSource(container){
	...
	function setTiledOverBackground(what,layer,matrixSet,setRendered){
		log("setTiledOverBackground("+what+","+layer+","+matrixSet+","+setRendered+")","info");
		if(what && rasterOverBackground===null){
			...
			if(tiled_matrixset!=null && tiled_layer_name!=null){
					...
					$http({method: "GET", url:urlWMTS+"?service=WMTS&VERSION=1.0.0&REQUEST=GetCapabilities"})
						.success(function(data){
										//try{
										...
										rasterOverBackground.setSource(new ol.source.WMTS(/** @type {!olx.source.WMTSOptions} */ (options)));
					});
					tileRendered = true;
					map.addLayer(rasterOverBackground);

			}else{
				log("TILED BACKGROUND ERROR: tiled_layer_name or tiled_matrixset not defined","error");
			}
		}else{
	...
	function addTemporalGeometry(geometry,source,removeAfter){
		log("addTemporalGeometry("+source+","+removeAfter+")","info",geometry);
  ...
  function getMultipleSelect(what){
		...
		if(trackingPosition===null){
			...
			trackingPosition.on('change:position', function() {
				...
				if(!geoLocalizedNotificate){
					$rootScope.$broadcast('geoLocalizeEvent',{evt:'GEOLOCATED',coordinates:coordinates});
			...
			trackingPosition.on('change', function() {
				...
				if(who==='remote'){
					$rootScope.$broadcast('geoLocalizeEvent',{evt:'GEOLOCATION_CHANGE',coordinates:trackingPosition.getPosition()});
				}
			});

			trackingPosition.on('error', function(error) {
				$rootScope.$broadcast('geoLocalizeEvent',{evt:'GEOLOCATION_ERROR'});
				log("trackPosition() -> geolocation error: "+error.message),"warn";
			});
			if(who==='local'){
				//markers & temporal geometry
	...
	function toogleUsersUbication(users){
		if(remoteTrackVectorLayer===null){
			...
			$rootScope.$broadcast('visualizationTools',{'key':'usersUbication', 'value': true});
		}else{
			...
			$rootScope.$broadcast('visualizationTools',{'key':'usersUbication', 'value': false});
		}
	}

	function addFeautureRemoteUser(user){
		var remotePositionFeature = new ol.Feature({name:user.socket_id});
		remotePositionFeature.setStyle(new ol.style.Style({
															image: new ol.style.Circle({
															radius: 6,
															fill: new ol.style.Fill({
																color: '#cc337c'
		...
		remotePositionFeature.setGeometry(user.coordinates ? new ol.geom.Point(user.coordinates) : null);
	...
	function updateFeautureRemoteUser(user){
		if(remoteTrackVectorLayer!=null){
			...
			remoteTrackVectorSource.getFeatures().forEach(function (ftr) {
				if(ftr.get('name')===user.socket_id){
					ftr.setGeometry(user.coordinates ? new ol.geom.Point(user.coordinates) : null);
				}
			});
		}
	...
	function offlineConfigure(what,ajax_target,project_name,token,offline_background,useConfirm,filters,availableFilterForLayers){
		...
		$http.post(baseHref+"ajax.offline.php", data2send,{
			...
			mapOffline.offlineConfigure(map,what,ajax_target,project_name,token,userName,bgOptions,useConfirm,filters,availableFilterForLayers);
		});
	...
	function OfllineGetAvailableGeoJson(ajax_target,project_name,token){
		...
		mapOffline.getAvailableGeoJson(ajax_target,project_name,token,map,false);
	}
	...
	function setOnlineMode(){
    	...
    	raster.setSource(null);
			...
			if(project.use_tiled_background && tileRendered){
				setTiledOverBackground(true,tiled_layer_name,tiled_matrixset,true);
			};
			reloadDisplayedLayers();
	...
	function loadGenericOfflineStyles(){
		...
		if(!genericOfflineStylesLoaded){
			mapOffline.readOfflineStyleGenericJsMethods(project.project_name+"_qgis2web_expressions.js",function(err,_genericStyles){
				if(err){
				...
				if(_genericStyles){
					...
					log("readOfflineStyleGenericJsMethods: no genericStyles","warn");
				}
			});
		}
	}
	...
	function photosSavePicture(ajax_target,visit_id,preview,layer,metadata,callback){
		if(appOnline){
			mapPhotos.savePicture(ajax_target,visit_id,preview,layer,metadata,callback);
		}else{
  ...
  function getFeatureTypeFromWFS(availableLayers){
  							...
  							if(typeof json.schema.complexType.complexContent.extension!="undefined"){
  								if(typeof json.schema.complexType.complexContent.extension.sequence!="undefined"){
  									if(typeof json.schema.complexType.complexContent.extension.sequence.element!="undefined"){
  										for(var i=0;i<json.schema.complexType.complexContent.extension.sequence.element.length;i++){
  											retorno.push(json.schema.complexType.complexContent.extension.sequence.element[i].name);
  										}
  									}
  								}
  							}
    ...
    if(typeof filter_expl_id!="undefined"){
			if(layerAvailableFilters.indexOf("expl_id")>-1){
				if(filter!=""){
					filter += ' AND ';
				}
				if(filter_expl_id.length>0){
					filter += '\"expl_id\" IN ( '+filter_expl_id.join(' , ')+' )';
				}else{
					filter += '\"expl_id\" IN ( -1 )';
				}
			}
		...
		if(typeof filter_state!="undefined"){
			if(layerAvailableFilters.indexOf("state")>-1){
				if(filter!=""){
					filter += ' AND ';
				}
				if(filter_state.length>0){
					filter += '\"state\" IN ( '+filter_state.join(' , ')+' )';
				}else{
					filter += '\"state\" IN ( -1 )';
				}
			}
    ...
    if(typeof filter_species!="undefined"){
      if(layerAvailableFilters.indexOf("species")>-1){
        if(filter!=""){
            filter += ' AND ';
        }
        if(filter_species.length>0){
            filter += '\"species_id\" IN ( '+filter_species.join(' , ')+' )';
        }else{
            filter += '\"species_id\" IN ( -1 )';
        }
      }
    }

    if(typeof filter_campaign!="undefined"){
      if(layerAvailableFilters.indexOf("campaign_id")>-1){
        if(filter!=""){
          filter += ' AND ';
        }
        if(filter_campaign.length>0){
          filter += '\"campaign_id\" IN ( '+filter_campaign.join(' , ')+' )';
        }else{
          filter += '\"campaign_id\" IN ( -1 )';
        }
      }
    }


		if(typeof lot_id!=null){
			if(layerAvailableFilters.indexOf("lot_id")>-1){
				if(filter!=""){
					filter += ' AND ';
				}
	...
	function zoomIn(){
		if(map.getView().getZoom()<25){
			...
			log("zoomIn() -> max level reached "+map.getView().getZoom(),"warn");
	...
	function zoomOut(){
		if(map.getView().getZoom()>min_zoom_level){
			...
			log("zoomOut() -> minimum level reached "+map.getView().getZoom(),"warn");
			//should disable button
		}
		setZoomButtons();
	}
	...
	function setZoomLevelAndCenter(level,center){
		...
		try{
			if(level>min_zoom_level && level<25){
	...
	function setZoomButtons(){
		...
		$rootScope.$broadcast('zoomButtons',{'zoomIn':zoomIn,'zoomOut':zoomOut});
	}
	...
	function zoomToHiglightedFeature(geomString){
		...
		if(typeof geomString!="undefined" && !geomString){
			if(typeof mapSelectTool.getSelectedFeauture().getGeometry().getExtent()=="object"){
				map.getView().fit(mapSelectTool.getSelectedFeauture().getGeometry().getExtent(), map.getSize());
			}
		}else{
			var format					= new ol.format.WKT({});
			var geom2Hightlight	= format.readGeometry(
	...
	function getZoomFromResolution(){
		return map.getView().getZoomForResolution(map.getView().getResolution());
	}
	...
	function toggleCoordinates(){
		...
		if(!controlExists){
			...
			$rootScope.$broadcast('visualizationTools',{'key':'showCoordinates', 'value': true});
		}else{
			map.removeControl(controlExists);
			localStorage.setItem('showCoordinates', 0);
			$rootScope.$broadcast('visualizationTools',{'key':'showCoordinates', 'value': false});
		}
	}
	...
	function toggleScale(){
		...
		if(!controlExists){
			...
			$rootScope.$broadcast('visualizationTools',{'key':'showScale', 'value': true});
		}else{
			map.removeControl(controlExists);
			localStorage.setItem('showScale', 0);
			$rootScope.$broadcast('visualizationTools',{'key':'showScale', 'value': false});
		}
	}
