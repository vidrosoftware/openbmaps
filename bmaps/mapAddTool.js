...
angular.module('app').factory('mapAddTool', ['$http','$rootScope', function ($http,$rootScope) {
	...
	function init(_map,_epsg,_viewProjection,_viewResolution,_vectorSource,_token,_app_name,_urlWMS,_geom_colors,_localized_strings){
		map								= _map;
		epsg							= _epsg;
		...
		if(typeof localized_strings.CLICK_TO_CONTINUE_DRAWING_POLYGON!="undefined"){
	...
	function addPoint(coordinates,type){
		...
		if(canAddPoint){
			...
			moveendEvent			= map.on('moveend', function() {
				log("addPoint: moveend "+map.getView().getCenter()+", canAddPoint: "+canAddPoint+", type: "+type);
				iconFeature.getGeometry().setCoordinates(map.getView().getCenter());
			});
