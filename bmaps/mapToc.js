...
angular.module('app').factory('mapToc', ['$http','$rootScope','mapFactory', function ($http,$rootScope,mapFactory) {
	...
	function init(tocOptions,_mc,_user_permissions){
		...
		setTimeout(function(){
			...
			if(queryTable){
				...
				if(queryPolId){
					$rootScope.$broadcast('queryEvent',{'evt':'found','pol_id':queryPolId,'id_name':getParameterByName('id_name'),'db_table':queryTable,'layer':layerName});
				}
			}
		},1100);
		...
		if(useCookies){
		    ...
		    if(queryTable){
					resetActiveLayer();
					let layerName = getLayerNameByLayerTable(queryTable);
					setActiveLayer(getObjectLayerByLayerName(getLayerNameByLayerTable(layerName)));
			...
			if (typeof mc.layers[i].Layer != 'undefined'){
				for (var s=0;s<mc.layers[i].Layer.length;s++){
					if(mc.layers[i].Layer[s].Name===item.Name){
						for(var p=0;p<properties.length;p++){
							mc.layers[i].Layer[s][properties[p].name] = properties[p].value;
						}
			...
			if(!item.isContainer){
				if(item.Name!=mapFactory.getActiveLayerName()){
					...
					if(index!=null){
						...
						log("mark current active layer as active "+mapFactory.getActiveLayerName());
					}
					//get legend
	...
	function addRemoveContainer(item,index,active){
		log("addRemoveContainer(index: "+index+", active: "+active+")",item);
		//first Level
		for (var i=0;i<mc.layers.length;i++){
			if(mc.layers[i].Name===item.Name){
	...
	function selectNextAvailableActiveLayer(){
		log("selectNextAvailableActiveLayer()",mapFactory.getLayersDisplayed());
		resetActiveLayer();
	...
	function addRemoveLayerFromCookie(obj,index,what){
      ...
      if(obj.isContainer === false) {
		...
		if(useCookies){
			log("addRemoveLayerFromCookie useCookies layers_cookie: ",layers_cookie);
			var cookieName = 'bmaps_'+project_id+"_tocCookie";
    ...
    var cookieName = 'bmaps_'+project_id+"_tocCookie";
		...
		try{
			...
			if(cookie.layers.length>0){
				setTimeout(function(){
					let storedLayer = localStorage.getItem("bmaps_"+project_id+"_tocCookieSelectedLayer");
					if(storedLayer){
					...
					$rootScope.$broadcast('legendEvent',{item:cookie.layers[0].item,event:'show'});
				},1000);
			}
			if(typeof cookie.zoom_level!="undefined"){
				mapFactory.setZoomLevelAndCenter(cookie.zoom_level,cookie.center);
...
//			container = mc.layers[i];
	...
	function _markContainerAsVisible(data){
		...
		if(childsVisible===data.childs.length && childsVisible>0){
			setLayerProperties(data.container,[{name:'isSelected',value:true}]);
			addRemoveLayerFromCookie(data.container,getLayerIndex(data.container.Name),true);
		}else{
			setLayerProperties(data.container,[{name:'isSelected',value:false}]);
			addRemoveLayerFromCookie(data.container,getLayerIndex(data.container.Name),false);
		}
	}
