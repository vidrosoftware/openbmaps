(function() {
	...
	angular.module('app').factory('mapOffline', ['$http','$rootScope','$window','mapStorage', function ($http,$rootScope,$window,mapStorage) {
		...
		function init(_proxyUrl,_touchDevice,_localized_strings,_project_id){
			log("init("+_proxyUrl+","+_touchDevice+","+_project_id+")","info");
			proxyUrl					= _proxyUrl;
		...
		/*	if(getUrl.pathname.split('/')[1]==="dev"){
			...
			setupPrototypes();
			setTimeout(function(){
				$rootScope.$broadcast('offlineDownloadEvent',{evt:"setDownloadButtons",selectArea:true,startDownload:false,selectingArea:false,showAreas:true,downloading:false,donwloadLayersBt:false});
			},1000);
		}
		...
		function getOfflineInfo(){
			...
			var data = {
				'offlineBackground':			 	mapStorage.getItem(storeName+'_offlineBackground'),
				'background_extent':			 	mapStorage.getItem(storeName+'_bgExtent'),
				'background_stored_date': 	formatDate(mapStorage.getItem(storeName+'_bg_stored_date')),
				'offlineData': 							mapStorage.getItem(storeName+'_offlineData'),
				'geojson_layers':						mapStorage.getItem(storeName+'_geojson_layers'),
				'geojson_stored_date':			formatDate(mapStorage.getItem(storeName+'_geojson_stored_date')),
				'visits_stored_date':				mapStorage.getItem(storeName+'_visits_stored_date')
			}
		...
		function storeProjectInfo(projectInfo,capabilities){
			...
			mapStorage.setItem(storeName+"_projectInfo",JSON.stringify(projectInfo));
		}

		function storeCapabilities(capabilities){
			log("storeCapabilities()","info",capabilities);
			mapStorage.setItem(storeName+"_capabilities",JSON.stringify(capabilities));
		}

		function getCapAndPi(project_id){
			log("getCapAndPi()","info",project_id);
			storeName					= "bmaps_"+project_id
			return {'projectInfo':JSON.parse(mapStorage.getItem(storeName+"_projectInfo")),'capabilities':	JSON.parse(mapStorage.getItem(storeName+"_capabilities"))};
		}
		//****************************************************************
		...
		function getStrings(project_id){
			...
			storeName					= "bmaps_"+project_id
			return {'strings':JSON.parse(mapStorage.getItem(storeName+"_strings"))};
		}
		...
		function getAvailableGeoJson(_ajax_target,_project_name,_token,_map,useConfirm,filters,availableFilterForLayers){
			log("getAvailableGeoJson("+_ajax_target+","+_project_name+","+_token+")","info",{'useConfirm':useConfirm,'filters': filters});
			if(map===null){
				map = _map;
			}
			...
			if(useConfirm){
				if(typeof localized_strings.OFFLINE_DOWNLOAD_LAYERS_ALERT!="undefined"){
					if (!confirm(localized_strings.OFFLINE_DOWNLOAD_LAYERS_ALERT)) {
						$rootScope.$broadcast('offlineDownloadEvent',{evt:"cancelDownload",selectArea:true,startDownload:false,selectingArea:false,showAreas:false,downloading:false,donwloadLayersBt:false});
						return;
					}
				}else{
			...
			$rootScope.$broadcast('offlineDownloadEvent',{evt:"startDownloadLayers",selectArea:false,startDownload:false,selectingArea:true,showAreas:false,downloading:true,donwloadLayersBt:false});
      ...
      data2send.append('filters', JSON.stringify(filters));
			data2send.append('availableFilterForLayers', JSON.stringify(availableFilterForLayers));
			$http.post(
					ajax_target,
					data2send,
					{
				...
				if(data.status==="Accepted"){
					...
					_proceedToDownloadGeoJsons(downloadableGeoJsons,data.message.hash);
				}else{
				...
				$rootScope.$broadcast('offlineDownloadEvent',{evt:"setDownloadButtons",selectArea:true,startDownload:false,selectingArea:false,showAreas:false,downloading:false,donwloadLayersBt:false});
			});
		...
		function _proceedToDownloadGeoJsons(downloadableGeoJsons,hash){
			...
			log("_proceedToDownloadGeoJsons("+hash+") result:","info",downloadableGeoJsons);
		...
		function callbackDownloadGeoJsonOk(layer_name){
			...
			$rootScope.$broadcast('offlineDownloadEvent',{evt:"geoJSONStored","name":layer_name.replace(_hashToDownload, '')});
			if(downloadableGeoJsons.length>0){
		...
		function callbackDownloadGeoJsonKo(layer_name,msg,data){
			...
			$rootScope.$broadcast('offlineDownloadEvent',{evt:"geoJSONStoredError","name":layer_name.replace(_hashToDownload, ''),"error":data});
		}

		function downloadGeoJsonLayer(layer_name,okCb,koCb){
			log("downloadGeoJsonLayer("+layer_name+")","info",_hashToDownload);
			var url = domainName+"bmaps/offline/"+layer_name;
			$rootScope.$broadcast('offlineDownloadEvent',{evt:"geoJSONStartDownload","name":layer_name.replace(_hashToDownload, '')});
			$http({method: "GET", url:url})
				.success(function(data){
					log("downloadGeoJsonLayer("+layer_name+") result: ","info",data);
					//store flags
					setTimeout(function(){
						var layer_name_to_store 	= layer_name.replace(_hashToDownload, '');
						var extension 						= layer_name_to_store.split(".");
						if(extension[extension.length-1]==="json"){
							log("downloadGeoJsonLayer("+layer_name+") store JSON","info",data);

							mapStorage.setItem(storeName+"_geojson_stored_date",new Date());
							var geojson_stored_layers = JSON.parse(mapStorage.getItem(storeName+"_geojson_layers"));
							...
							mapStorage.setItem(storeName+"_geojson_layers",JSON.stringify(geojson_stored_layers));
							setTimeout(function(){
									...
									setTimeout(function(){
										...
										mapStorage.setTile(layer_name_to_store, JSON.stringify(data) ,function(err, content) {
											if(err){
												log("downloadGeoJsonLayer ("+layer_name+") error inserting","error",err);
												koCb(layer_name,err,data);
											}else{
												log("downloadGeoJsonLayer ("+layer_name_to_store+") successfully stored localForage","success");
												okCb(layer_name);
											}
										});
									},100);
					...
					//	mapStorage.removeFileFromFileSystem(layer_name.replace(_hashToDownload, ''))
						mapStorage.writeFileOnFileSystem(layer_name.replace(_hashToDownload, ''),data,function(err,msg){
							if(err){
					...
					koCb(layer_name,"downloadGeoJsonLayer("+layer_name+") error on get",url);
				});
		}
		...
		function readOfflineGeoJSON(layer,cb){
			...
			request.onsuccess = function(event) {
				...
				request2.onsuccess = function(event3) {
					log("request2.onsuccess","success",event3.target.result);
					try{
						...
						log("error parsing stored geoJSON for layer: "+layer,"error",e)
		...
		function offlineConfigure(_map,what,ajax_target,project_name,token,_userName,_bgOptions,useConfirm,filters,availableFilterForLayers){
			log("offlineConfigure()","info",{'_map':_map,'what':what,'ajax_target':ajax_target,'project_name':project_name,'token':token,'_userName':_userName,'_bgOptions':_bgOptions,'useConfirm':useConfirm});

			if(map===null){
				map = _map;
			}
			userName 	= _userName;
			$rootScope.$broadcast('offlineDownloadEvent',{evt:"setDownloadButtons",selectArea:false,startDownload:false,selectingArea:false,showAreas:false,downloading:true,donwloadLayersBt:true});
			getExtenToDownload();
			if(what==="background"){
				if(_bgOptions===null){
					//default background
					_bgOptions = {
	 						bg_base_url : "https://geoserveis.icgc.cat/icc_mapesmultibase/utm/wmts/service",
	 						layer: 				'topogris',
	 						matrixSet: 		'UTM25831'
	 				};
				}
				...
				getAvailableGeoJson(ajax_target,project_name,token,map,useConfirm,filters,availableFilterForLayers);
			}
		}
		...
		function renderWMTSLayers(bgOptions){
			...
			$http({method: "GET", url:bgOptions.bg_base_url+"?service=WMTS&VERSION=1.0.0&REQUEST=GetCapabilities"})
					.success(function(data){
						try{
		...
		function save(capabilities,bgOptions) {
			...
			if (layers.length == 0) {
				...
				mapStorage.setItem(storeName+'_bgExtent',JSON.stringify(currentExtent));
					...
					if(!offlineBackgroundDisplayed){
						function getAllItems(db,callback) {
							...
							trans.oncomplete = function(evt) {
								...
								var storedCapabilities 		= mapStorage.getItem(storeName+'_bgCapabilities');
								var parser 								= new ol.format.WMTSCapabilities();
...
//****************************************************************
		...
		function addVisit(epsg,pol_id,coordinates,layer,callback){
				...
				if(visits!==null){
					...
					//no visits stored
					mapStorage.setItem(storeName+'_visits',JSON.stringify([newElement]));
		...
		function getVisitEvents(visit_id){
			...
			var visit_events 			= JSON.parse(mapStorage.getItem(storeName+'_visit_events'));
			var returnData 				= [];
		...
		function addVisitInfo(visit_id,heading,formData,images,compasses,photo,okCb,koCb){
			...
			if(visit_events!==null){
				...
				mapStorage.setItem(storeName+'_visit_events',JSON.stringify(visit_events));
			}else{
				mapStorage.setItem(storeName+'_visit_events',JSON.stringify([data2send]));
			}
			mapStorage.removeItem(storeName+'_visits_stored_date');
			mapStorage.setItem(storeName+'_visits_stored_date',new Date());
			okCb();
		...
		function removePhoto(photo_id,callback){
			...
			mapStorage.removeTile("photo_"+photo_id,function() {
				...
				mapStorage.setItem(storeName+'_photos',JSON.stringify(currentPhotos));
				callback();
			});
...
function renderAreaToDownLoad(meters){
	...
	if(selectingAreaToDownload){
		...
		$rootScope.$broadcast('offlineDownloadEvent',{evt:"setDownloadButtons",selectArea:true,startDownload:true,selectingArea:true,showAreas:true,downloading:false,donwloadLayersBt:true});
