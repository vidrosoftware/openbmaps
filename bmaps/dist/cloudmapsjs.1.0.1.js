	...
	function Controller(mapFactory,loggerService, fileReader,$rootScope,$scope,$http,socketFactory) {
		...
		$scope.setActiveLayer	= function(item,index){
			...
			if(!item.isContainer){
				...
				$scope.addRemoveLayer(item,index);
			}
		}
		...
		function loadBgMaps(){
			$http.post(baseHref+'includes/background_maps.json', {}).success(function (data) {
				log("loadBgMaps() result:",data);
				mc.available_bg_layers	= data.bgs;
			}).error(function (error) {
				log("loadBgMaps() error:",error);
			});	
		}
	    ...
	    $scope.$on('capabilities', function(event, data) {
			...
			formatLayers(data.Capability.Layer.Layer);
...
angular.module('app').factory('mapFactory', ['$http','$rootScope','mapMeasureTools','mapSelectTool','mapAddTool', function ($http,$rootScope,mapMeasureTools,mapSelectTool,mapAddTool) {
	...
	function setBackGroundMap(map){
		if(map==="OSM"){
			...
			var source = new ol.source.TileWMS({url:'http://mapcache.icc.cat/map/bases/service?layers='+map+'&srs='+epsg});	
		}
