...
angular.module('app').factory('mapPhotos', ['$http','$rootScope', function ($http,$rootScope) {
	...
	function savePicture(ajax_target,visit_id,preview,layer,metaData,callback){
		log("savePicture("+ajax_target+","+visit_id+","+layer+")","info",{'metaData':metaData});
				...
				data2send,
				{
					...
					headers: {'Content-Type': undefined
						...
						if(data.status==="Accepted"){
							...
							callback("Error requesting savePicture","Error requesting savePicture",data);
						}
	...
	function showPhoto(ajax_target,photo_id,cbOk,cbKo){
		...
		$http.get(
			ajax_target+photo_id+"&nocache="+noCache,
					{
						transformRequest: angular.identity,
						headers: {'Content-Type': undefined
					}
	...
	function deletePhoto(ajax_target,photo_id,layer_name,pol_id,tableIdName,cbOk,cbKo){
