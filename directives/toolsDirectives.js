	...
	return {
		...
		link: 			function(scope, _elem, attrs) {
									...
									scope.updateAction 	= function(){
										$rootScope.executeUpdate(scope.changeAction,scope.fieldName,scope.value);
									}
	...
	return {
		...
		link: 			function(scope, _elem, attrs) {
									...
									scope.updateAction 	= function(){
										...
										if(typeof directiveData.widgetAction!="undefined" && directiveData.widgetAction!=null){
											$rootScope.widgetAction(directiveData.widgetAction,scope.fieldName,scope.value);
										}else{
											...
											$rootScope.executeUpdate(scope.changeAction,scope.fieldName,scope.value,geomString);
										}
									}
								}
		}
...
app.directive('formCheck', function($rootScope) {
	...
	return {
		...
		link: 			function(scope, _elem, attrs) {
									...
									if(directiveData.value=="t" || directiveData.value=="true" || directiveData.value=="True" || directiveData.value==true){
										scope.value 				= true;
									...
									scope.clickCheck = function(name) {
										...
										if(typeof directiveData.changeAction!="undefined"){
											$rootScope.executeUpdate(directiveData.changeAction,scope.fieldName,$rootScope.feature[name]);
										}
									}
								}
		}
		...
		link: 			function(scope, _elem, attrs) {
									...
									scope.getData				= function(val){
										if(val.length>=scope.threshold){
											...
											return $rootScope.getTypeAheadDataAction(scope.tabName,scope.fieldName,scope.getDataAction,val,scope.searchService);
										}
									}

									scope.selectAction 	= function($item,$model,$label,$event){
										//console.log($item,$model,$label,$event,directiveData.selectAction)
										$rootScope.feature[scope.fieldName] = scope.value;
										$rootScope.selectTypeAheadAction(scope.selectDataAction,$item,$model,$label,$event);
									}
								}
		}
});
	...
	return {
		...
		link: 			function(scope, _elem, attrs) {
									...
									if(directiveData.value!=""){
										...
										init_date 												= new Date(splitDate[2]+"/"+splitDate[1]+"/"+splitDate[0]);
									}
		...
		link: 			function(scope, _elem, attrs) {
									...
									scope.selectAction 					= function(item){
										...
										if(typeof item.sys_id!="undefined"){
											...
											$rootScope.$broadcast('logEvent',{evt:"NO sys_id",extradata:item,file:'toolsDirectives.js formList',level:'warn'});
										}
