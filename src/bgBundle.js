...
var _richLogger = _interopRequireDefault(require("../richLogger"));
...
var _self = null,
    ...
    ol = null,
    sensibilityFactor = 3,
    // sensibility factor to increase tolerance on clic/touch
_logger = null;
var _available_bg_layers = [];
...
/*#__PURE__*/
function () {
  function bgsBmaps(options, _ol) {
    _classCallCheck(this, bgsBmaps);
    ...
    _logger.info(_self._fileName, "Bmaps Backgrounds v.".concat(_package.version, " loaded"));
  ...
  _createClass(bgsBmaps, [{
    key: "setEpsg",
    value: function setEpsg(epsg) {
      _logger.info(_self._fileName, "setEpsg(".concat(epsg, ")"));

      _epsg = epsg;
    }
  }, {
    key: "setProjectBgsProperty",
    value: function setProjectBgsProperty(property, value) {
      _logger.info(_self._fileName, "setProjectBgsProperty(".concat(property, ")"), value);

      _mainBackgrounds[property] = value;
    }
  }, {
    key: "getProjectBgsProperty",
    value: function getProjectBgsProperty(property) {
      _logger.info(_self._fileName, "getProjectBgsProperty(".concat(property, ")"));

      return _mainBackgrounds[property];
    }
  }, {
    key: "loadProjectBgs",
    value: function loadProjectBgs() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _axios["default"].post("".concat(_options.baseHref, "/ajax.projects.php"), dataToSend).then(function (response) {
    ...
    key: "getBackground",
    value: function getBackground(mapName) {
      _logger.info(_self._fileName, "getBackground(".concat(mapName, ")"));

      var properties = _self._getBackgroundProperties(mapName);

      return new Promise(function (resolve, reject) {
        if (typeof mapName === 'undefined') {
          reject(new Error('no mapName'));
        } else if (mapName.substring(0, 7) === 'custom_') {
          _axios["default"].get("".concat(_options.baseHref, "/background.proxy.php?service=WMTS&VERSION=1.0.0&REQUEST=GetCapabilities")).then(function (response) {
            _logger.success(_self._fileName, "getBackground ".concat(mapName), response);
            ...
            options.urls[0] = "".concat(_options.baseHref, "background.proxy.php");
            resolve(new ol.source.WMTS(options));
          })["catch"](function (error) {
            _logger.error(_self._fileName, "getBackground ".concat(mapName), error);
          ...
          switch (mapName) {
            ...
            case 'diba':
              resolve(_self._generateTileWMSsource({
                url: "http://sitmun.diba.cat/wms/servlet/BUE1M?layers=BUE1M_211L,BUE1M_111L,BUE1M_211P,BUE1M_111T,BUE1M_111P,BUE1M_311T&srs=".concat(_epsg)
              }));
              break;

            case 'vol2016':
              resolve(_self._generateTileWMSsource({
                url: "https://192.168.21.48/cgi-bin/vol2016/qgis_mapserv.fcgi?layers=vol2016&srs=".concat(_epsg)
              }));
              break;

            case 'vol2012':
              resolve(_self._generateTileWMSsource({
                url: "https://192.168.21.48/cgi-bin/vol2012/qgis_mapserv.fcgi?layers=vol2012&srs=".concat(_epsg)
              }));
              break;
            ...
            default:
              _self.setDynamicBg(mapName, properties).then(function (response) {
                resolve(response);
              })["catch"](function (error) {
                reject(error);
              });
    ...
    key: "setDynamicBg",
    value: function setDynamicBg(mapName, properties) {
      _logger.info(_self._fileName, "DynamicBg ".concat(mapName), properties);

      return new Promise(function (resolve, reject) {
        if (properties.type === 'WMS') {
          var opt = JSON.parse(properties.options);
          opt.url = "".concat(properties.url, "?srs=").concat(_epsg);
          resolve(new ol.source.TileWMS(opt));
        } else if (properties.type === 'WMTS') {
          _axios["default"].get("".concat(properties.url, "?service=WMTS&VERSION=1.0.0&REQUEST=GetCapabilities")).then(function (response) {
            _logger.success(_self._fileName, "DynamicBg ".concat(mapName), response);
            ...
            _logger.error(_self._fileName, "DynamicBg ".concat(mapName), error);
    ...
    key: "_generateTileWMSsource",
    value: function _generateTileWMSsource(obj) {
      return new ol.source.TileWMS(obj);
    }
  }, {
    key: "_getBackgroundProperties",
    value: function _getBackgroundProperties(id) {
      _logger.info(_self._fileName, "getBackgroundProperties(".concat(id, ")"));
    ...
    key: "getInfoFromBackground",
    value: function getInfoFromBackground(bgId, source, coordinates, mapData) {
      _logger.info(_self._fileName, "getInfoFromBackground(".concat(bgId, ")"), {
        'source': source,
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (bgInfo.info_options) {
          try {
            var url = source.getGetFeatureInfoUrl(coordinates, Math.min(Math.max(mapData.viewResolution * sensibilityFactor, 1), 1), mapData.projection, JSON.parse(bgInfo.info_options));

            _logger.info(_self._fileName, 'getInfoFromBackground url', url);

            _axios["default"].get("".concat(_options.baseHref, "/backgroundInfo.proxy.php?url=").concat(url)).then(function (response) {
          ...
          reject({
            'error': 'info not available for this background',
            "type": 'background'
          });
        }
      });
...
exports["default"] = bgsBmaps;
window.bgsBmaps = bgsBmaps;

},{"../richLogger":29,"./package":28,"axios":2}],2:[function(require,module,exports){
"use strict";
